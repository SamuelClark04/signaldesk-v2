...
// first break through (strategies: equity-swing, crypto-swing).
const PIVOT_K = 3;
...
function checkTarget(bars, entry, target, fmt = (x) => x) {
  ...
  if (target > r.price) {
    return { ok: false, resistance: r, text: `The target ${fmt(target)} is above the major daily resistance ${fmt(r.price)} (${r.kind} ${day}).` };
  }
  return { ok: true, resistance: r, text: `The target ${fmt(target)} sits below the major daily resistance ${fmt(r.price)} (${r.kind} ${day}).` };
}
