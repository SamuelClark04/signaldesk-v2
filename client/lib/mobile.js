...
//   Bottom tab bar   Today · Opportunities [Approvals badge] · Portfolio · Journal · Settings
//                    (Phase 64B: every top-level section; Approvals is the Opportunities
//                    sub-tab), fixed above the home indicator (env(safe-area-inset-bottom))
...
(() => {
  ...
  const ICONS = { // 24x24 stroke icons
    ...
    journal: 'M5 4h11a3 3 0 013 3v13H8a3 3 0 01-3-3zM5 17a3 3 0 013-3h11M9 8h6',
  ...
  const TABS = [['today', 'Today', 'today'], ['setups', 'Opportunities', 'opportunities?tab=setups'],
    ['portfolio', 'Portfolio', 'portfolio'], ['journal', 'Journal', 'journal'], ['settings', 'Settings', 'settings']];
  ...
  function sync(tab, state) {
    build();
    const active = tab === 'opportunities' ? 'setups' : tab;
    ...
    document.querySelectorAll('.m-tab').forEach((b) => {
      ...
      if (b.dataset.key === 'setups' && waiting) { // setups waiting for approval (Opportunities → Approvals)
