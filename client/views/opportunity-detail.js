...
(() => {
  ...
  function center(o, { livePrice }) {
    ...
    const change = livePrice > 0 && o.entryPrice > 0 ? livePrice / o.entryPrice - 1 : null;
    return el('section', { className: 'opp-center' }, [
      el('header', { className: 'opp-head' }, [
        ...
        el('span', { className: 'opp-meta', textContent: `${o.setupType || 'Setup'} · ${o.timeframe || '—'} · ${o.strategyId}` }),
      ...
      levelChart(o, livePrice),
      el('div', { className: 'opp-thesis' }, [
  ...
  function scenarioTable(o) {
    const s = o.scenarios || {};
    const rows = [['Stop', s.stop], ['T1', s.t1], ['T2', s.t2]].filter(([, v]) => v);
    if (!rows.length) return el('p', { className: 'opp-muted', textContent: 'No scenarios available.' });
    return el('table', { className: 'data-table opp-scenarios' }, [
      ...
      el('tbody', {}, rows.map(([name, v]) => el('tr', {}, [
  ...
  function actions(o, ctx) {
    ...
    const primary = el('button', {
      type: 'button',
      className: `btn ${live ? 'btn-live' : 'btn-primary'} opp-go`,
      textContent: label,
      disabled: !ctx.online || ctx.busy || liveOptions,
      title: !ctx.online ? 'Offline' : '',
    });
    primary.onclick = () => ctx.onApprove(o, { live, broker });
    const dismiss = el('button', { type: 'button', className: 'btn', textContent: 'Dismiss', disabled: !ctx.online || ctx.busy });
    dismiss.onclick = () => ctx.onDismiss(o);
    return el('div', { className: 'opp-actions' }, [
      el('div', { className: `opp-venue${live ? ' is-live' : ''}`, textContent: live ? `LIVE · real order at ${broker}` : `Paper · ${broker} mode is Paper` }),
      primary,
      dismiss,
    ]);
  }
  ...
  function right(o, ctx) {
    ...
    return el('aside', { className: 'opp-right' }, [
      ...
      kv('Entry range', `${px(o.entryZone.min, o)} – ${px(o.entryZone.max, o)}`),
      kv('Invalidation (stop)', px(o.invalidation, o), 'text-short'),
      kv('Take profit 1', t[0] ? px(t[0].price, o) : '—', 'text-long'),
      kv('Take profit 2', t[1] ? px(t[1].price, o) : '—', 'text-long'),
      ...(o.optionsData ? [kv('Structure', `${(o.optionsData.legs || []).map((l) => `${l.side} ${l.strike}${l.type === 'put' ? 'P' : 'C'}`).join(' / ')} · ${o.optionsData.debit} debit`)] : []),
      el('h3', { className: 'opp-section', textContent: 'Risk' }),
      kv('Planned size', size(o)),
      kv('Risk amount', `${money(o.dollarRisk)}${riskPct}`),
      kv('Net reward / risk (T1)', rr),
      kv('Fee drag', Number.isFinite(o.feeDrag) ? `${o.feeDrag.toFixed(2)}R` : '—'),
  ...
  SD.oppDetail = { center, right, displaySymbol };
})();
